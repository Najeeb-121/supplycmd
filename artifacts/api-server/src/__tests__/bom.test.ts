...
describe('BOM Propagation (Phase 1)', () => {
  ...
  it('Test H: zero component quantity remains zero', () => {
    const boms = [
      { id: 10, odooBomId: 110, parentSkuId: 70, parentBomQty: 1, isActive: true }
    ];
    ...
    expect(graph[70].lines[0].componentQty).toBe(0);
  });
  it('Test I: multiple active BOMs for the same parent do not silently overwrite', () => {
    const boms = [
      { id: 20, odooBomId: 201, parentSkuId: 80, parentBomQty: 1, isActive: true },
      { id: 21, odooBomId: 202, parentSkuId: 80, parentBomQty: 1, isActive: true }
    ];
    ...
    expect(() => buildBOMGraph(boms, bomLines)).toThrow(
      "MULTIPLE_ACTIVE_BOMS_FOR_PARENT"
    );
  });
});
